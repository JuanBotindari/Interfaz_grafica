...
export function CircleNodeView({ node, isSelected, onMouseDown, onContextMenu }: Props) {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: "26px", fontWeight: 800, color: themeConfig.colors.textPrimary, textAlign: "center", padding: "0 24px", lineHeight: 1.2 }}>
        {node.name}
      </span>
      <span style={{ fontSize: "16px", fontWeight: 600, color: "#22c55e" }}>{node.status}</span>
    </div>
  );
}